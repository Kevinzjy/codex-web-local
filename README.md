...
- **Directory picker UX:** In the New chat folder picker, the directory list is the only scrolling region. `Manual path` and action buttons stay visible even when there are many entries.
...
- **Composer commands:** `/status` is handled client-side and renders a local status card. `@` path completion supports both relative and absolute queries (`@.` / `@../` / `@/`), preserves the user’s path style when inserting, and shows right-column size metadata for files only. `!<shell command>` runs on the host via `/bin/bash -lc` in the active thread cwd and renders output as a dedicated command panel (`You ran ...` + output block). Long command output stays fully available inside a scrollable panel (default 24 visible lines). Code/output blocks include a top-right one-click copy button.
